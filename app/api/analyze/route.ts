...
export async function POST(request: Request) {
  ...
  const system = [
    ...
    "返回一个 JSON 对象，字段：entities(string[]), action(string), object(string), suggestedEvent(huakun|beite|new|uncertain), matchReason(string), claims([{text,quote,kind(事实陈述|观点|推测|传闻),relation(支持|反驳|更正|仅提及)}])。",
  ].join("\n");
